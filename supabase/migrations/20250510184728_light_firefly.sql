/*
  ...
  1. New Tables
    - `users`
      ...
      - Supports multiple user roles (client, transporter, admin, finance)
    - `wallets`
      - User wallet for managing funds
      - Tracks balance and transaction history
    - `vehicle_types`
      - Available vehicle types for transport
      - Includes pricing and capacity information
    - `jobs`
      - Transport job listings
      - Links clients and transporters
    - `tracking_records`
      - Real-time location tracking for jobs
    - `payments`
      - Payment transactions
      - Supports multiple payment types
    - `escrows`
      - Holds funds for jobs until completion
    - `ratings`
...
CREATE POLICY "Clients can create jobs" ON jobs
  ...
  WITH CHECK (auth.uid() = client_id AND role() = 'authenticated');
